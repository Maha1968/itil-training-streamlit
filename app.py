...
def load_training_slides(path: str = "itil_trg.pdf") -> List[Dict[str, Any]]:
    # PDF is loaded from Google Drive preview, create slide metadata
    # Using a reasonable default number of pages (can be adjusted)
    total_pages = 50
    slide_width = 1920
    slide_height = 1080
    aspect_ratio = 16/9
    ...
    log_event("info", f"Created {len(slides)} slide entries for Google Drive PDF")
    return slides
...
# =====================================================
    ...
    if st.session_state.slides_loaded and st.session_state.slides:
        ...
        # Google Drive preview URL with page parameter
        pdf_url = f"https://drive.google.com/file/d/1reuk-JLnou4D3rgDbhIA7fhDhqw8RZm9/preview#page={current_page}"
