...
def load_policy_text(path: str = "itil_trg.pdf") -> str:
    # Load PDF from Google Drive and extract text for mock test
    try:
        pdf_bytes = download_pdf_from_google_drive()
        ...
        if not full_text.strip():
            log_event("warning", "PDF text extraction returned empty, but continuing")
            return ""
        return full_text
    except Exception as e:
        log_event("warning", f"Failed to extract text from PDF: {e}, continuing without text")
        return ""
...
# =====================================================
def download_pdf_from_google_drive() -> bytes:
    """Download PDF from Google Drive in memory."""
    ...
    try:
        ...
        try:
            alt_url = "https://drive.google.com/file/d/1reuk-JLnou4D3rgDbhIA7fhDhqw8RZm9/export?format=pdf"
            response = requests.get(alt_url, stream=True, timeout=30)
            response.raise_for_status()
            return response.content
        except Exception as e2:
            raise ValueError(f"Failed to download PDF from Google Drive: {e2}")
...
def load_training_slides(path: str = "itil_trg.pdf") -> List[Dict[str, Any]]:
    # Download PDF from Google Drive and convert to images
    pdf_bytes = download_pdf_from_google_drive()
    ...
    # Store images in session_state for caching
    if "slide_images" not in st.session_state:
        st.session_state.slide_images = images
    
    if not slides:
        raise ValueError("No slides were created from PDF")
    
    log_event("info", f"Loaded {len(slides)} slides from Google Drive PDF")
    return slides
...
def load_training_content():
    ...
    try:
        # Try to load slides first (this is the critical part)
        slides = load_training_slides("itil_trg.pdf")
        
        # Try to extract text for mock test (non-critical, can fail gracefully)
        try:
            policy_text = load_policy_text("itil_trg.pdf")
        except Exception as text_error:
            log_event("warning", f"Text extraction failed but continuing: {text_error}")
            policy_text = ""
        
        log_event("info", f"Training content loaded from Google Drive. {len(slides)} slides")
...
if "show_mock_test_prompt" not in st.session_state:
    st.session_state.show_mock_test_prompt = False
...
# =====================================================
    ...
    if not st.session_state.slides_loaded:
        with st.spinner("⏳ Loading training content from Google Drive... Please wait."):
            ...
            if error:
                ...
                st.success("✅ Training content loaded from Google Drive.")
    ...
    if st.session_state.slides_loaded and st.session_state.slides:
        ...
        if slide_images and current_idx < len(slide_images):
            current_image = slide_images[current_idx]
            
            # Center the slide image
            col_left, col_center, col_right = st.columns([1, 3, 1])
            with col_center:
                st.image(current_image, use_container_width=True)
        else:
            st.warning("⏳ Loading slide...")
        
        # Navigation buttons
        col1, col2, col3 = st.columns([1, 1, 1])
        
        with col1:
            if st.button("⬅ Previous", disabled=(current_idx == 0), key="prev_slide"):
                if current_idx > 0:
        ...
        if is_last_slide:
            ...
            if st.button("✅ Complete Training", type="primary", key="complete_training"):
                st.session_state.slides_completed = True
                st.session_state.current_step = "completed"
                st.session_state.show_mock_test_prompt = True
...
# =====================================================
    ...
    if st.session_state.show_mock_test_prompt:
        st.markdown("### Would you like to take the mock test?")
        col1, col2 = st.columns(2)
        
        with col1:
            if st.button("Yes", type="primary", key="start_mock_test"):
                st.session_state.current_step = "mock_test"
                st.session_state.show_mock_test_prompt = False
                st.rerun()
        
        with col2:
            if st.button("No", key="skip_mock_test"):
                st.session_state.show_mock_test_prompt = False
                name = st.session_state.user_email.split('@')[0] if st.session_state.user_email else "Employee"
                st.info(f"**Congratulations {name}**\n\nYou have successfully completed the ITIL Foundation Training\n\n**{COMPANY_NAME}**\n\n{datetime.now().strftime('%B %d, %Y')}")
    
    if not st.session_state.show_mock_test_prompt and not st.session_state.mock_test_submitted:
        name = st.session_state.user_email.split('@')[0] if st.session_state.user_email else "Employee"
        st.info(f"**Congratulations {name}**\n\nYou have successfully completed the ITIL Foundation Training\n\n**{COMPANY_NAME}**\n\n{datetime.now().strftime('%B %d, %Y')}")
...
# =====================================================
    ...
    if st.session_state.mock_test_questions and not st.session_state.mock_test_submitted:
        ...
        if current_q_idx < len(questions):
            ...
            with col1:
                if st.button("⬅ Previous", disabled=(current_q_idx == 0 or time_up), key=f"prev_q_{current_q_idx}"):
                    if current_q_idx > 0:
            ...
            with col3:
                if st.button("Next ➡", disabled=(current_q_idx >= len(questions) - 1 or time_up), key=f"next_q_{current_q_idx}"):
    ...
    if st.session_state.mock_test_submitted:
        ...
        if passed:
            ...
            if st.button("Try Again", type="primary", key="retry_mock_test"):
                st.session_state.mock_test_questions = []
                st.session_state.mock_test_submitted = False
                st.session_state.mock_test_idx = 0
                st.session_state.mock_test_answers = {}
