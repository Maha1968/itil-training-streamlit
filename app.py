...
def load_policy_text(path: str = "itil_trg.pdf") -> str:
    # PDF is loaded from Google Drive, return empty string for text extraction
    # Mock test will need to be handled differently if policy text is required
    return ""
...
def download_pdf_from_google_drive() -> bytes:
    ...
    # Convert preview URL to export format
    pdf_url = "https://drive.google.com/file/d/1reuk-JLnou4D3rgDbhIA7fhDhqw8RZm9/export?format=pdf"
    try:
        ...
        try:
            alt_url = "https://drive.google.com/uc?export=download&id=1reuk-JLnou4D3rgDbhIA7fhDhqw8RZm9"
...
def load_training_slides(path: str = "itil_trg.pdf") -> List[Dict[str, Any]]:
    ...
    # This function now returns slide metadata, actual images are stored in session_state
...
# =====================================================
    ...
    if st.session_state.slides_loaded and st.session_state.slides:
        ...
        if slide_images:
            ...
            st.warning("⏳ Loading slides...")
        
        # Calculate total slides
        total_slides = len(slide_images) if slide_images else len(slides)
        max_idx = total_slides - 1
        ...
        with col2:
            st.write(f"Slide {current_idx + 1} / {total_slides}")
        
        with col3:
            if st.button("Next ➡", disabled=(current_idx >= max_idx), key="next_slide"):
                if current_idx < max_idx:
                    st.session_state.current_slide_idx = current_idx + 1
                    st.rerun()
        
        # Complete Training button (only on last slide)
        is_last_slide = (current_idx >= max_idx)
        if is_last_slide:
